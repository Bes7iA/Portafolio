// SVGs de los logos de cada red social, con sus colores de marca oficiales.
// Se definen aquí (fuera de la función renderContacto) para que no se
// regeneren en cada llamado a la función — son datos estáticos que nunca
  ...
  `).join('')
    ...
    <section id="contacto" class="bg-arcade-navy/45 backdrop-blur-md border-[3px] border-arcade-border shadow-[6px_6px_0px_#000000] p-6 sm:p-10 space-y-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-2">
        ${links}
        <button id="btn-abrir-contacto"
                class="bg-white text-black border-[3px] border-black shadow-[4px_4px_0px_#000000] font-black uppercase text-xs p-4 flex flex-col items-center justify-center gap-2 transition-all duration-150 hover:bg-arcade-aqua-hover hover:shadow-[2px_2px_0px_#000000] hover:translate-x-0.5 hover:translate-y-0.5 text-center group">
          <span class="group-hover:scale-110 transition-transform">${iconos.gmail}</span>
          <span class="text-xs font-black">EMAIL DIRECTO</span>
          <span class="text-[10px] font-mono lowercase text-gray-700">Enviar mensaje</span>
        </button>
      </div>
    </section>
  `
